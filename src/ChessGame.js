...
const API_URL = process.env.REACT_APP_API_URL || 'https://chess-alpha-beta.onrender.com';
...
function ChessGame() {
  ...
  const startNewGame = async () => {
    try {
      const response = await axios.get(`${API_URL}/new-game?agent=${selectedAgent}`);
  ...
  const fetchLegalMoves = async () => {
    try {
      const response = await axios.get(`${API_URL}/legal-moves`);
  ...
  const onDrop = async ({ sourceSquare, targetSquare }) => {
    ...
    try {
      ...
      if (response.data.game_over) {
        ...
        await delay(1000);
        const botMoveResponse = await axios.get(`${API_URL}/bot-move?agent=${selectedAgent}`);
